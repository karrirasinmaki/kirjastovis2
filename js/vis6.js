        ...
        return function(d) {
          ...
          node = {
            ...
            teema: d.Lookup_Teema,
						...
						hvuosi: d.Lookup_Vuosi,
//						tila: d.Tila,
						lkm: d.lkm,
						avi: d.Lookup_Aluehallintovirasto,
...
//    };
   ...
   //        return '#cfcfcf'; // harmaa get_color_map_Lookup_Teema
	  //       return '#404040'; // harmaa get_color_map_Lookup_Teema
		//			 return this_get_color_map_Lookup_Teema; // harmaa get_color_map_Lookup_Teema
         };
        })
         ...
         return d.radius;
			  ...
			  this.circles.enter().append("circle").attr("r", 0).attr("opacity", 1).style("fill", (function(chart, colorBy, _this) {
         return function(d) {
           return '#cfcfcf'; // harmaa
		//       return '#404040'; // harmaa get_color_map_Lookup_Teema
		//			 return chart.color_by(colorBy); // harmaa get_color_map_Lookup_Teema
         };
        })(this)).attr("stroke-width", 2).attr("stroke", (function(_this) {
         return function(d) {
           return '#404040'; // tummanharmaa
    ...
    BubbleChart.prototype.do_split = function(accessor, options) {
      ...
      force_map.forEach((function(_this) {
        return function(force) {
          ...
          force.force.gravity(0).theta(1.0).charge(charge).chargeDistance(Infinity).friction(0.88).on("tick", function(e) {
...
//            radius: _this.estimate_circle_diameter(leaves) / 2
						radius: _this.estimate_circle_diameter(leaves) / 2.7
          };
        };
      ...
      prev_y = options.view_by_amount != null ? -90 : -60;
			  if (func === 'kunta') {
			    sort = options.sort != null ? options.sort : function(b, a) {
          return d3.ascending(parseFloat(groups.get(b).sum), parseFloat(groups.get(a).sum));
					};
			  } else {
...
//				if (searchTerm.length > 0 && match >= 0) this.show_details(d, i, this);
//				if (searchTerm.length > 0 && match === 1) element.show_details;
//				  return d.searched = true;
//        else this.hide_details(this);
      ...
      return this.circles.each(function(d) {
        ...
        if (match) {
					element.style("stroke-width", 2.0).style("stroke-opacity", 1).style("fill-opacity", 1);
...
//					});
          ...
          d.searched = false;
					  element.style("stroke-width", 2.0).style("stroke-opacity", 0.1).style("fill-opacity", 0.1);
//						element.style("stroke-width", 2.0).style("fill-opacity", 0.1);
//            element.attr("r", 0);
        }
      });
    };
...
//				}
//				content += "<a href= "+data.link+" target="_blank">" + "<br/>" ;
//        content += "<a href="/release-notes">What's new in 3.2</a>";
				...
				content += "<span class=\"name\">Kohderyhm&auml;t:</span><span class=\"value\"> " + data.kohderyhma + "</span><br/>";
...
//				content += "<span class=\"name\">Budjetti:</span><span class=\"value\"> " + (addCommas(data.value)) + " euroa" + "</span><br/>";
				...
				if ('ontouchstart' in document.documentElement) content += "<br><a href='" + url + "' target='_blank'>Tutustu hankkeeseen tarkemmin</a>";
//        content += "</div>";
//			}
    ...
    for (_i = 0, _len = allValuesArray.length; _i < _len; _i++) {
      ...
      switch (value) {
        ...
        case "Kokoelmat":
          ...
          break;
				case "Lukemisen edist�minen":
          color_map[value] = '#33a02c';
          break;
				case "Mediaskasvatus":
          color_map[value] = '#cab2d6';
          break;
				case "Oppimisymp�rist� ja yhteis�llisyys":
          color_map[value] = '#a6cee3';
          break;
				case "Palvelujen kehitt�minen":
          color_map[value] = '#1f78b4';
          break;
				case "Strateginen kehitt�minen":
    ...
    root.get_chart = (function(_this) {
      ...
      return function(colorBy) {
        if (colorBy === '') {
				...
				} else if (colorBy === "Lookup_Maakunta") {
					return chart.color_by(colorBy);
        } else {
          return chart.color_by(colorBy);
        }
      };
	//		return function(colorByteema) {
  //      if (colorByteema === "Lookup_Maakunta") {
	//				return chart.color_by(colorByteema);
  //      }
  //    };
		})(this);
    ...
    $('#viz_nav_container .viz_nav').on('click', function(e) {
      ...
      return chart.updateSearch(searchTerm);
       ...
       disable_search: false,
			 ...
			 no_results_text: "Etsim��si ei l�ydy"
    });
